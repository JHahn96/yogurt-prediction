...
for(i in seq(1,length(dfs))){
  df<-dfs[[i]]
  for(k in seq(1,length(dfs))){
    df.pred<-dfs.pred[[i]]
    if (df$article_name[1]==df.pred$article_name[1]){
      dfs[[i]]<-dplyr::bind_rows(df, df.pred)
    }
  }
}
```
...
ts.pred.gesund <- trainingg
ts.pred.joghurt <- trainingJ
ts.pred.joghurt.price <- trainingJP
ts.pred.milk.price <- trainingMP
```
