...
library(forecast)
auto.arima(trainingg, ic = "aic") #0/0/1
auto.arima(trainingJ, ic = "aic") #1/1/0

ges<-arima(x=trainingg,order=c(0,0,1))
Jog<-arima(x=trainingJ,order=c(1,1,0))
ges.pred <- predict(ges, n.ahead = 12)
Jog.pred <- predict(Jog, n.ahead = 12)

##Jonathan, ges.pred und Jog.pred sind die predicted values. Allerdings ändert autoarim sich immer wieder. Im Moment sind es monatliche Daten, müsste man wieder auf wöchentlich hochrechnen
...
# refactor columns for prophet package
  ...
  prod <- as.character(unlist(df_final[1,3]))
  model <- "lin1"
