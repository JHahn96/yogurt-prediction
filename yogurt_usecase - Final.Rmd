...
#m <- add_country_holidays(m, df)
  ...
  training$sales<-df_final$sales[1:(len-ntest)]
  testing$sales<-df_final$sales[(len-ntest+1):len]
...
for(i in seq(1,length(prediction))){
  ...
  model_names<- c(
    'pred_prophet1' = sprintf("Prophet 1 \n MAPE: %.2f (Train) - %.2f (Test)",
  ...
  g1 <- ggplot(aes(y=value, x=ds, color=variable), data=testqstat) +
    ...
    labs(title=paste(c("Modelcomparison of ", yogurt, ", with MAPE Statistics."), sep=""), 
