...
for(i in seq(1,length(prediction))){
  ...
  testqstat<- data.table::melt(prediction[[i]], id="ds", measure=c("pred_ProphetScoresM", 
                                                           "pred_prophet2", 
                                                           "pred_glm", 
                                                           "pred_xgboost"))
  model_names<- c(
    'pred_ProphetScoresM' = sprintf("ProphetScoresM \n MAPE: %.2f (Train) - %.2f (Test)",
                     mape(training$y, training$pred_ProphetScoresM),
                     mape(testing$y, testing$pred_pred_ProphetScoresM)),
    
    'pred_prophet2' = sprintf(
      "Prophet 2 \n MAPE: %.2f (Train) - %.2f (Test)",
                     mape(training$y, training$pred_prophet2),
                     mape(testing$y, testing$pred_prophet2)),
    
    'pred_glm' = sprintf("GLM \n  MAPE: %.2f (Train) - %.2f (Test)",
                     mape(training$y, training$pred_glm),
                     mape(testing$y, testing$pred_glm)),  
